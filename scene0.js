class scene0 extends Phaser.Scene {
  ...
  //telanave, musica 2f spawnar asteroide a cada 3 seg, animacao do fundo, som de coleta de combustivel
  preload() {
    this.load.plugin(
  ...
  hitAsteroid(player, asteroid) {
    (player.disableBody(true, true), this.scene.start("game-over"));
  }
  ...
  spawnAsteroid() {
    ...
    const y = Phaser.Math.Between(0, this.game.config.heigth); // Top of the screen

    // Use a custom class or the default group functionality
    const asteroid = this.asteroidGroup.create(x, y, "asteroideum");

    // Configure the newly created enemy
    asteroid.setBounce(1);
    asteroid.setCollideWorldBounds(true);
    asteroid.setVelocityY(Phaser.Math.Between(100, 200));

    this.time.addEvent({
      delay: 3000, // Call every 3 seconds
